const filmy = [
	...
	},
	{
		...
		nazev: 'Maják',
		plakat: {
			url: 'https://image.pmgstatic.com/cache/resized/w420/files/images/film/posters/164/095/164095239_7f77b1.jpg',
			sirka: 420,
			vyska: 593,
		},
		ochutnavka: 'Hypnotické klaustrofobické drama o strážcích majáku.',
		popis:
			'Hypnotické klaustrofobické drama o strážcích majáku (Robert Pattinson a Willem Dafoe) na konci 19. století. Soužití větrem, léty a nadměrným pitím sužovaného námořníka a jeho nového pomocníka se zvrhne do šílenství a paranoie, když nováček odhalí okolnosti záhadného osudu svého předchůdce. Zajímavá kompozice záběrů, černobílý obraz a dramatický zvuk vytvářejí strašidelnou atmosféru.',
		premiera: '2019-10-18',
	},
]

let filmId = location.hash.substring(1);
let chosenFilm = filmy.find((film) => film.id == filmId);

document.querySelector("#nameHeader").innerHTML = chosenFilm.nazev;
document.querySelector("#descriptionText").innerHTML = chosenFilm.popis;

let poster = document.querySelector("#filmPoster");
poster.src = chosenFilm.plakat.url;

let date = dayjs(chosenFilm.premiera);
let today = dayjs();
...
if (diff > 0) {
	premiereText = `je za ${diff} ${dayText}`;
} else if (diff < 0) {
	diff = diff * -1; 
	premiereText = `bylo před ${diff} ${dayText}`;
} else {
	premiereText = `je dnes`;
}

document.querySelector("#premiera").innerHTML = `Premiéra <strong>${date.format("D.M.YYYY")}</strong>, což ${premiereText}.`

let currentRating = 0;

let ratingFunction = (starCount) => {
	let counter = 0;
...
for (let star of document.querySelectorAll(".fa-star")) {
	star.addEventListener("click", () => {
		let index = star.innerHTML;
		
		ratingFunction(index);
		currentRating = index;
	});
...
document.querySelector("#note-form").addEventListener("submit", (event) => {
	...
	let checkboxInput = document.querySelector("#terms-checkbox");
...
let videoPlayerContainer = document.querySelector("#prehravac");

let timeout;

let hideControlsFunction = () => {
	return setTimeout(() => {
		document.querySelector(".player-controls").classList.add("hidden");
	}, 3000);
}

let showControlsFunction = () => {
